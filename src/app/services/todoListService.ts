...
export class TodoListService {
  ...
  public addItem(title: string, description?: string): boolean {
    if (title.trim()) {
      ...
      this.httpClient.post(this.apiUrl, newItem).subscribe({
        error: (err) => {
  ...
  public updateItem(updatedItem: TodoListItem) {
    this.httpClient.put(`${this.apiUrl}/${updatedItem.id}`, updatedItem).subscribe({
      error: (err) => {
  ...
  public deleteItem(itemId: string) {
    this.httpClient.delete(`${this.apiUrl}/${itemId}`).subscribe({
      error: (err) => {
